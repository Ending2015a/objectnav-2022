...
#   https://github.com/anheidelonghu/ACNet/blob/master/ACNet_models_V1.py
...
@registry.register.semantic_model('acnet')
class ACNet(BaseSemanticModel):
  def __init__(
    self,
    num_classes: int = 40,
    pretrained: bool = False
  ):
    """ACNet model (ResNet50 ver.). Borrowed from the official impl.:
    https://github.com/anheidelonghu/ACNet/blob/master/ACNet_models_V1.py
  ...
  def forward(
    ...
    if use_checkpoint:
      ...
      fuses = checkpoint(self.encoder, rgb, depth)
      out = checkpoint(self.decoder, *fuses)
    else:
      fuses = self.encoder(rgb, depth)
      out = self.decoder(*fuses)
    return out